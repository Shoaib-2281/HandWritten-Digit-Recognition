...
def destroy_widget(widget):
    widget.destroy()
     #Captures the current canvas as a PostScript image.
     #Converts the image to PNG format using PIL (Python Imaging Library).
     #Resizes the image to 28x28 pixels and converts it to grayscale.
     #Reshapes and normalizes the image data to match the model's input requirements.
     #Uses the loaded model to predict the digit in the image.
     #Displays the predicted digit and its accuracy in the GUI.
def pred_digit():
    global no,no1
